...
public class SoundRecorderService extends Service implements Player.PlayerListener,
            ...
            if (!isStorageMounted()) {
                ...
                if (requestAudioFocus()) {
                    res = mRecorder.startRecording(getApplicationContext(), params, fileSizeLimit,
                            getRecordingFileDescriptor(params));
    ...
    public boolean isCurrentFileWaitToSave() {
        if (!mRecordSaving && mParcelFileFd != null) {
            return true;
        }
        return false;
    }
    ...
    protected FileDescriptor getRecordingFileDescriptor(RecordParams params) {
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd_HH.mm.ss.SSS");
        String prefix = dateFormat.format(new Date(System.currentTimeMillis()));
        ContentValues cv = new ContentValues();
        cv.put(MediaStore.Audio.Media.DISPLAY_NAME, prefix);
        cv.put(MediaStore.Audio.Media.TITLE, prefix);
        cv.put(MediaStore.Audio.Media.MIME_TYPE, params.mMimeType);
        ...
        try {
            ...
            LogUtils.e(TAG, "open recording file fail");
        }
    ...
    private void closeRecordingFile() {
        try {
            ...
            LogUtils.e(TAG, "close recording file fail");
        }
    }
    ...
    private void updateMediaDatabase() {
        ...
        if (ret == -1) {
            LogUtils.e(TAG, "update record file info error");
            closeRecordingFile();
            return;
        }
